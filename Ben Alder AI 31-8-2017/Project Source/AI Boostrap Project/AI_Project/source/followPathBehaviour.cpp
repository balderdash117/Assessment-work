...
void followPathBehaviour::update(GameObject * obj, float deltaTime)
{
	...
	if (!path.empty())
	{
		...
		if (m_currentPathNodeIndex - path.size() == 0 || path.size() == 0)
		{
...
void followPathBehaviour::draw(GameObject * obj, aie::Renderer2D * renderer)
{
	if (overlayClass::getGraphShowing())
	{
		...
		for (auto iter = path.begin(); iter != path.end(); ++iter, ++index)
		{
			...
			{
				renderer->setRenderColour(0.0f, 0.0f, 1.0f, 0.75f);
			}
			else if (obj->getObjectType() == 1)
			{
				renderer->setRenderColour(1.0f, 0.0f, 0.0f, 0.75f);
			}
